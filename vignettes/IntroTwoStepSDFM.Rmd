...
[^seed_call_cross_val]: Even though these random candidates are not drawn within `C++`, the function still requires a `seed` parameter. This is done for reproducibility, as we store the provided `seed` inside the function call object of the return object of `crossVal()`. Note that only the provided `seed` parameter will govern the internal RNG state of the function. Further, the internal `set.seed(seed)` call will not alter the global RNG state of the current `R` session, as the RNG state will be reset upon exit. 
