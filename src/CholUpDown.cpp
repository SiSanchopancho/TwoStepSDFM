...
)
{
    ...
    Eigen::MatrixXd L_DL = L.block(index, index, NN - index, NN - index);
    int M = L_DL.cols();
    ...
    for (int m = 0; m < M; ++m)
    {
        ...
        L(Eigen::seq(index + m + 1, index + M - 1), index + m) = (l_mm / L_DL(m, m)) * L_DL(Eigen::seq(m + 1, M - 1), m) + omega.tail(M - m - 1) * ((l_mm * omega(m)) / gamma);
