...
)
{
	...
	if (steps == INT_MIN && (selected.array() >= N).all() && l1.array().isNaN().any()) {

		Eigen::MatrixXd ridge_add = Eigen::MatrixXd::Identity(N, N) * l2;
		for (int k = 0; k < K; ++k)
		{
			// create target vector
			y_j = X * A.col(k);
			B.col(k) = (X.transpose() * X + ridge_add).llt().solve(X.transpose() * y_j);
		}
	}
	else {
	...
	// Check whether LARS is even required
